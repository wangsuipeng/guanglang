<template>
    <div class="inspectionPlan">
        ...
        <el-table :data="planData" border style="width: 100%;">
            ...
            <el-table-column align="center" prop="pollingTime" label="巡检时间"></el-table-column>
            <el-table-column align="center" prop="presult" label="巡检结果"></el-table-column>
            <el-table-column align="center" prop="accessory" label="附件" width="110"></el-table-column>
        ...
        ></el-pagination>
        <el-dialog title="新增巡检记录" center :visible.sync="dialogPlanVisible" width="35%">
                ...
                <el-form-item label="巡检时间">
                    <el-col :span="24">
                        <el-date-picker
                            type="date"
                            placeholder="选择日期"
                            v-model="pollForm.pollingTime"
                            style="width: 100%;"
                        ></el-date-picker>
                    </el-col>
                </el-form-item>
                <el-form-item label="巡检结果">
                    <el-select style="width: 100%" v-model="pollForm.presult" placeholder="请选择资产类型">
...
export default {
    ...
    methods: {
        ...
        getPlanData() {
            $axios({
                url:
                    "bjpolling/pollingRecord/findAllByKey?pageNumber=" +
                    this.currentPage +
                    "&pageSize=" +
        ...
        addPoll(row) {
            if (!row.id) {
                ...
                this.pollForm = {
                    id: row.id,
                    title: row.title, // 标题
                    assetName: row.assetName, // 资产/备品名称
                    context: row.context, // 巡检内容
                    pollingUser: row.pollingUser, // 巡检人
                    pollingUserId: row.pollingUserId, // 巡检人id
                    fillUserId: row.fillUserId, // 填写人id
                    fillUser: row.fillUser, // 填写人
                    pollingTime: row.pollingTime, // 巡检时间
                    presult: row.presult, // 巡检结果
                    pollingPeriod: row.pollingPeriod, // 巡检周期
                    remark: row.remark, // 备注
                    accessory: row.accessory, // 附件
                    pollingPlace: row.pollingPlace //巡检位置
                };
                this.dialogPlanVisible = true;
            }
        },
        ...
        savePoll() {
            ...
            this.pollForm.pollingPeriod = parseInt(this.pollForm.pollingPeriod);
            for (var i = 0; i < this.personData.length; i++) {
                if (this.personData[i].nickname == this.pollForm.pollingUser) {
            ...
            if (this.pollForm.id == "" || this.pollForm.id == null) {
                $axios({
                    url: "bjpolling/pollingRecord/save",
                    method: "post",
                    data: this.pollForm
                })
                    .then(result => {
                ...
                $axios({
                    url: "bjpolling/pollingRecord/update",
                    method: "post",
                    data: this.pollForm
                })
                    .then(result => {
        ...
        deletePoll(row) {
            ...
            this.$confirm("您确定要删除本条巡检计划吗?", "提示", {
                ...
                .then(() => {
                    $axios({
                        url: "bjpolling/pollingRecord/delete",
                        method: "post",
                        data: pollId
                    })
                        .then(result => {
