<template>
  <div class="assetsParameter">
    <div class="row_">
      <div class="col1_">
        <el-cascader v-model="typeId" :options="options" :props="props" filterable change-on-select clearable
                     @change="handleChange"></el-cascader>
        <el-input style="width: 200px" placeholder="请输入内容" clearable v-model="search"></el-input>
        <el-button type="primary" icon="el-icon-search" @click="getList()">搜索</el-button>
      </div>
      <div class="col2_">
        <el-button type="primary" @click="dialogTypeVisible = true">设置资产类型</el-button>
    ...
    <div class="table_">
      <el-table ref="multipleTable" v-loading="loading" stripe border :data="tableData" tooltip-effect="dark"
        ...
        <el-table-column label="资产编码" align="center" prop="coding"></el-table-column>
    ...
    <el-dialog :title="isEditAssets ? '编辑资产' : '添加资产'" center width="900px" :before-close="handleClose"
        ...
        <el-row :gutter="30">
          <el-col :span="12">
            <div class="grid-content bg-purple-light">
              <el-form-item label="资产类型" prop="typeId" :label-width="formLabelWidth">
          ...
          <el-col :span="12">
            <div class="grid-content bg-purple-light">
              <el-form-item label="资产编码" prop="coding" :label-width="formLabelWidth">
    ...
    <el-dialog title="设置资产类型" center width="900px" :visible.sync="dialogTypeVisible">
      <div class="row_">
        <div class="col1_">
      ...
      <el-table :data="typeData" border :loading="typeLoading">
        ...
        <el-table-column align="center" prop="name" label="资产类型"></el-table-column>
    ...
    <el-dialog :title="isTypeEdit ? '编辑资产类型' : '添加资产类型'" center width="400px" :before-close="closeType"
               :visible.sync="dialogAddTypeVisible">
      <el-form :model="typeForm" :rules="typeRules" ref="typeRef">
        <el-form-item label="资产类型" prop="name" :label-width="formLabelWidth">
          <el-input v-model="typeForm.name"></el-input>
        </el-form-item>
        <el-form-item label="上级资产类型" prop="parentId" :label-width="formLabelWidth">
    ...
    <el-dialog title="导入" center width="900px" :visible.sync="dialogImportVisible">
      ...
      <el-row style="margin: 5px 0;font-size: 12px;color: #9d9d9d;">
        2.上传资产文件表头需包含资产编码、名称、型号、生产厂商、供应商名称、备选商名称、生产日期、供应日期、变更日期、保养期、保质期、预警时间
      </el-row>
      <div slot="footer" class="dialog-footer">
  ...
  export default {
    ...
    data() {
      return {
        ...
        isEditAssets: false,
        warningDateDay: '',
        action: `${API_PATH}bjeam/asset/importAssets`,
        QRcode: '',
        search: '',
        typeSearch: '',
        fileList: [],
        ...
        rules: {
          ...
          coding: [{required: true, message: '请输入资产编码', trigger: 'blur'},],
    ...
    methods: {
      ...
      saveAssetType(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (this.isTypeEdit) {
              $axios.post('bjeam/assetType/update', data).then(res => {
                if (res.code === 200) {
              ...
              delete data.id;
              $axios.post('bjeam/assetType/save', data).then(res => {
                if (res.code === 200) {
      ...
      getAssetList() {
        ...
        $axios.post(`bjeam/assetType/findAllByPage?pageNumber=${pageNumber}&pageSize=${this.AssetList.pageSize}&param=${this.typeSearch}`, {}).then(res => {
          if (res.code === 200) {
      ...
      saveAssets(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (this.isTypeEdit) {
              $axios.post('bjeam/asset/update', data).then(res => {
                if (res.code === 200) {
                  this.$refs[formName].resetFields();
              ...
              delete data.id;
              $axios.post('bjeam/asset/save', data).then(res => {
                if (res.code === 200) {
                  this.$refs[formName].resetFields();
      ...
      getAssetType() {
        $axios.post('bjeam/assetType/findAll').then(res => {
          if (res.code === 200) {
      ...
      delType(row) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          $axios.post(`bjeam/assetType/deleteById?id=${row.id}`, {}).then(res => {
      ...
      delTable(row) {
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          this.loading = true;
          $axios.post(`bjeam/asset/deleteById?id=${row.id}`, {}).then(res => {
            this.loading = false;
            if (res.code === 200) {
              this.getList();
              this.$message({type: 'success', message: '删除成功！'})
      ...
      getList() {
        this.loading = true;
        let type = this.typeId.slice(-1);
        $axios.post(`bjeam/asset/findAllByPage?pageNumber=${this.currentPage - 1}&pageSize=${this.tablePageSize}&typeId=${type}&param=${this.search}`, {}).then(res => {
          this.loading = false;
          if (res.code === 200) {
